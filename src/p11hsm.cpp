...
#define HSMLIB_PRODUCT_VERSION "2.5.0"
        ...
        unsigned long overwrite, unsigned long* h_cert)
{
    ...
    CK_ATTRIBUTE cert_template[] = {
      ...
      {CKA_SERIAL_NUMBER,     cert_serial,      cert_serial_len},
      {CKA_CERTIFICATE_TYPE,  &cert_type,       sizeof(cert_type)},
      {CKA_SUBJECT,           cert_subject,     cert_subject_len},
      {CKA_ISSUER,            cert_issuer,      cert_issuer_len},
      {CKA_VALUE,             cert_value,       cert_value_len},
      {CKA_TOKEN,             &b_token,         sizeof(b_token)},
      {CKA_PRIVATE,           &b_private,       sizeof(b_private)},
      {CKA_MODIFIABLE,        &b_modifiable,    sizeof(b_modifiable)},
      {CKA_ID,                cert_id,          cert_id_len},
      {CKA_LABEL,             cert_label,       cert_label_len}
    };

    CK_OBJECT_HANDLE h_obj;
    CK_ATTRIBUTE* p_attrib = cert_template;
    CK_ULONG pub_template_len = DIM(cert_template);

    rv = _p11->C_CreateObject(h_session, p_attrib, pub_template_len, &h_obj);
